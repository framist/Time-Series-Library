...
# 阶段 1 核心对照（Electricity/ECL：electricity.csv + Transformer）
#
# 说明：
# - electricity 数据值域很大（最高可到 1e6 量级），适合验证 no_scale 的稳定性与 prior 的无量纲化效果。
#
# 四组：
# - A：--no_scale + timeF
...
#
# prior 参数说明：
# - 需要设置环境变量 PRIOR_SCALE（标量）
#   例：PRIOR_SCALE="1528000" bash scripts/wvembs/forecast_electricity_stage01_core.sh
...
run_one () {
  ...
  python -u run.py \
    ...
    --root_path "${ROOT}/" \
    --data_path "${DATA}" \
    --model_id "WVEmbs_ECL_${MODEL}_${group}_${embed}" \
    --model "${MODEL}" \
    --data "${DATASET}" \
    --features "${FEATURES}" \
    --target "${TARGET}" \
